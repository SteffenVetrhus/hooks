...
the ``organizations`` collection.  Uses ``httpx.AsyncClient`` for non-blocking
I/O so that the FastAPI event-loop is never stalled by upstream requests.
"""
...
async def list_organizations(
    ...
    """
    Retrieve a paginated list of organizations from PocketBase.
...
async def get_organization(organization_id: str) -> dict[str, Any]:
    """
    Retrieve a single organization record by its PocketBase ID.

    Args:
        organization_id: The unique PocketBase record ID of the organization.

    Returns:
        dict[str, Any]: The raw JSON object representing the organization
        record.
    ...
    async with _get_client() as client:
        ...
        response = await client.get(
            f"{BASE_URL}/{organization_id}", headers=headers
        )
        response.raise_for_status()
        return response.json()


async def create_organization(data: dict[str, Any]) -> dict[str, Any]:
    """
    Create a new organization record in PocketBase.

    Args:
        data: A dictionary of field values for the new organization.  Must
              conform to the ``OrganizationCreate`` schema.

    Returns:
        dict[str, Any]: The newly created organization record as returned by
    ...
    Side Effects:
        Creates a new record in the PocketBase ``organizations`` collection.
    """
    async with _get_client() as client:
        headers = await _authenticate(client)
...
async def update_organization(
    ...
    """
    Update an existing organization record in PocketBase using PATCH semantics.
    ...
    Returns:
        dict[str, Any]: The updated organization record as returned by
        PocketBase.
    ...
    Side Effects:
        Modifies an existing record in the PocketBase ``organizations``
        collection.
    """
    async with _get_client() as client:
        headers = await _authenticate(client)
        response = await client.patch(
            f"{BASE_URL}/{organization_id}", json=data, headers=headers
        )
        response.raise_for_status()
        return response.json()
...
async def delete_organization(organization_id: str) -> None:
    """
    Delete an organization record from PocketBase.
    ...
    Side Effects:
        Permanently removes the record from the PocketBase ``organizations``
        collection.
    """
    async with _get_client() as client:
        headers = await _authenticate(client)
